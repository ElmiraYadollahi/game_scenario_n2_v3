				...
				for i in range(len(trajComplete2)):
					...
					for j in range(len(trajComplete2[i][0])):
						...
						if (j ) %4 == 0:
							...
							correctFlag = wordProc.readFromWordMatrix(correctPermission, j/4, i+1, breakPoint, REDButtonAllert, YELLOWButtonAllert, taskLevel_dict['CurrLevel'], ARTag, current_word_pub)
...
def IntroduceNao(childName):
	...
	story.setLanguage('English')
	#script =   ("\\rspd=90\\ Hello everyone \\pau=700\\ My name is Nao \\pau=500\\ I really like reading short stories"
	#			"\\rspd=90\\ Do you want to listen to them?"
	#			"\\rspd=90\\ sometimes I make mistakes, can you help me to correct them?"
	#			"\\rspd=90\\ If you want to read with me, please bring the book")
	#script = ("\\rspd=90\\ Hello")
	#story.say(script)
...
def main():
	...
	robot_instruction_pub = rospy.Publisher("instruction_state", String, queue_size=5)
	...
	childName = "Gabriel"
	child_name_pub.publish("Child Name")
