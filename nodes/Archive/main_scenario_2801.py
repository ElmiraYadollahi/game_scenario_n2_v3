...
def MoveBeadBackPublisher(value, maxRowLength, beadRadius, prevValue=0, rowNum=0):
	...
	for i in range (1, nbBead + 1):
		...
		print ("move the beads -------------------------------------- back time")


def MoveBeadPublisherRow2(value, maxRowLength, beadRadius, prevValue=0, rowNum=0):
	nbBead = 10
	maxRowLength = basic_data_dict['length']
	beadRadius = basic_data_dict['radius']
	yekan = row_count_dict['yekan']
	...
	for i in range(1, value + 1):
		...
		position_publish = rospy.Publisher('abacus/row' + str(rowNum)+ '/bead'+ str(movingBead) + '/setX', String, queue_size=1)
		position_publish.publish(str(movingLocation))
		print ("move the beads -------------------------------------- second row")
		#PointAtTheTablet("Right")


def MoveBeadPublisher(value, maxRowLength, beadRadius, turn, prevValue=0, rowNum=0):
	"""
	...
	if yekan == 0:
		for i in range(1, value + 1):
			movingBead = nbBead - i
			movingLocation = maxRowLength - (beadRadius * (i - 1))
			currentLocation = minRowLength + (beadRadius *  i - round(beadRadius/2))

			position_publish = rospy.Publisher('abacus/row' + str(rowNum)+ '/bead'+ str(movingBead) + '/setX', String, queue_size=1)
			position_publish.publish(str(movingLocation))
			print ("move the beads -------------------------------------- first time")
		...
		for i in range(1, value + 1):
			...
			if yekan + i <= nbBead:
				...
				position_publish = rospy.Publisher('abacus/row' + str(rowNum) + '/bead'+ str(movingBead) + '/setX', String, queue_size=1)
				position_publish.publish(str(movingLocation))
				print ("move the beads --------------------------------------second time")

	
	if value < 0:
		for i in range(1, (- value) + 1):
			movingBead = nbBead - (yekan - i + 1)
			movingLocation = minRowLength + (beadRadius * (nbBead - yekan + i )) - round(beadRadius/2)
	...
	"""if yekan == 0:
		...
		for i in range(1, value + 1):
			...
			if yekan + i <= nbBead:
				...
				if yekan + i == nbBead:
					movingBead = nbBead - 1
					movingLocation = maxRowLength - (beadRadius)
					position_publish = rospy.Publisher('abacus/row1/bead'+ str(nbBead - 1) + '/setX', String, queue_size=1)
					position_publish.publish(str(movingLocation))
					print ("move the beads --------------------------------------second row")
			...
			if yekan + i > nbBead:
				k = yekan + i - nbBead
				movingBead = nbBead -  k
				movingLocation = maxRowLength - (beadRadius * ( k - 1))
	...
	if value < 0:
		for i in range(1, (- value) + 1):
			movingBead = nbBead - (yekan - i + 1)
			movingLocation = minRowLength + (beadRadius * (yekan - i - 3/2))
...
def PointAtTheTablet(direction):
	global tracker
	global frame
	...
	curPose = motionProxy.getPosition("LHand", frame, useSensorValues)

	tracker.post.lookAt(position1, frame, maxSpeedHead, useWholeBody)
	tracker.pointAt("LArm", position1, frame, maxSpeedHead)
	print "pointing "
	...
	postureProxy.goToPosture("Crouch", 0.1)
...
def ProductionInteractionScenario(prodSeries, proOrder, productionBatch):
	...
	LookAtTheTablet()
	PointAtTheTablet("Right")
	lookingDirection = MoveBeadPublisher(firstValue, maxRowLength, beadRadius, turn)
	...
	if XsecondValue + firstValue < nbBead:
		if XsecondValue<0:
			...
			PointAtTheTablet("Right")
		MoveBeadPublisher(XsecondValue, maxRowLength, beadRadius, firstValue, turn)
		#rospy.Subscriber("target_pose", PoseArray, MoveRobotArm)
	...
	if XsecondValue + firstValue >= nbBead:
		...
		PointAtTheTablet("Right")
		MoveBeadPublisher(belowTen, maxRowLength, beadRadius, firstValue, turn)
		...
		PointAtTheTablet("Right")
		MoveBeadPublisherRow2(1, maxRowLength, beadRadius, 0, 1)
		AboveTen = XsecondValue - belowTen
		time.sleep(1.5)
		# Move the rest of the beads in the first row to find the final answer
		MoveBeadPublisher(AboveTen, maxRowLength, beadRadius, turn)
